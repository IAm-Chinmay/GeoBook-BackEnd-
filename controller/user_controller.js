...
const signUpUser = async (req,res,next) => {
    ...
    if(!errors.isEmpty()){
        next( new httpError("Please enter valid data in given feilds",422));
    }
    ...
    try{
        ...
        return next(new Http_error("Email retriving failed ",500))
    }
    ...
    if(emailExist){
      ...
      try{
       ...
       return next(new Http_error("Username retriving failed ",500))
   }
...
const loginUser = async (req,res,next) => {
    ...
    let isUser
     try{
            ...
            return next(new httpError("Email retriving failed",500))
        }
